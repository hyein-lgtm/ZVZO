...
def extract_items(scraped_data: dict) -> dict:
    ...
    # --- 매출 검증용 진단: 일자별 읽은 값을 그대로 찍어 ZVZO 표와 비교 ---
    try:
        lines = []
        for d in sorted(daily.keys()):
            v = daily[d]
            lines.append(f"  {d}  주문합계={v['sales']:,}  취소={v['cancel']:,}  순={v['net']:,}")
        total_sales = sum(v["sales"] for v in daily.values())
        total_net = sum(v["net"] for v in daily.values())
        dbg = (f"\n[매출검증] 수집일수 {len(daily)} · 주문합계총 {total_sales:,} · "
               f"순매출총 {total_net:,}\n" + "\n".join(lines))
        scraped_data["_진단"] = scraped_data.get("_진단", "") + dbg
    except Exception:
        pass
